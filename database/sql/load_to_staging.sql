CREATE OR REPLACE FUNCTION get_or_create_config_loadstaging(
...
CREATE OR REPLACE FUNCTION create_new_loadstaging_log(p_config_id INT)
RETURNS VARCHAR AS $$
DECLARE
    v_date_part TEXT;
    v_cnt INT;
...
BEGIN
    ...
    v_exec_id := 'LOD_STG_' || v_date_part || '_' || LPAD((v_cnt + 1)::TEXT, 3, '0');
...
CREATE OR REPLACE FUNCTION update_loadstaging_log_status(
    p_exec_id VARCHAR, 
...
CREATE OR REPLACE FUNCTION "public"."check_today_loadstaging_success"()
...
BEGIN
    ...
    RETURN EXISTS (
        ...
        WHERE c.config_name LIKE 'LOD_STG%' 
...
$BODY$
  ...
  CREATE OR REPLACE FUNCTION "public"."check_today_loadstaging_success_prefix"("p_process_prefix" varchar)
  RETURNS "pg_catalog"."bool" AS $BODY$
BEGIN
    RETURN EXISTS (
        SELECT 1 
