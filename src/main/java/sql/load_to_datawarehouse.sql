CREATE OR REPLACE FUNCTION create_new_log_loadwarehouse(p_config_id INT)
RETURNS VARCHAR AS $$
DECLARE
    v_date_part TEXT;
    v_cnt INT;
...
BEGIN
    ...
    -- Lưu ý: Vẫn cần phân tách theo định dạng cũ (dấu '-') để tìm MAX(số thứ tự)
    ...
    -- Tạo ID mới: LOD_DW-YYYYMMDD-XXX
    v_exec_id := 'LOD_DW_' || v_date_part || '_' || LPAD((v_cnt + 1)::TEXT, 3, '0');
...
CREATE OR REPLACE FUNCTION "public"."check_today_success_loadwarehouse"("p_process_prefix" varchar)
...
CREATE OR REPLACE FUNCTION update_log_status_loadwarehouse(
    p_exec_id VARCHAR, 
...
CREATE OR REPLACE FUNCTION get_or_create_process_loadwarehouse(
...
DECLARE
    v_id INT;
BEGIN
    -- 1. Tìm xem process đã tồn tại chưa
    SELECT config_process_id INTO v_id -- Giữ nguyên tên cột
    FROM config_process -- Giữ nguyên tên bảng
