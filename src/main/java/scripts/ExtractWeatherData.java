...
public class ExtractWeatherData {
    ...
    public static int extractWeatherToFile(
        ...
        try {
            ...
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(dbOutputPath))) {
                ...
                for (Location location : locations) {
                    try {
                        ...
                        if (response.statusCode() == 200) {
                            ...
                            failedLocations.add(location.getName() + " (" + location.getCode() + ") - HTTP " + response.statusCode());
                            failCount++;
                        }
                        ...
                        failedLocations.add(location.getName() + " (" + location.getCode() + ") - " + e.getMessage());
                        failCount++;
                    }
                }
                ...
                sendExtractReport(executionId, successCount, failCount, failedLocations);
     ...
     */
    private static void sendExtractReport(String executionId, int successCount, int failCount, List<String> failedLocations) {
        try {
            ...
            body.append("Extract Time: ").append(LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))).append("\n\n");
            body.append("--- SUMMARY ---\n");
